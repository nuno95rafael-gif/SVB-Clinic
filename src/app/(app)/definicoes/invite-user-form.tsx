"use client";

import { useActionState, useState } from "react";
import { inviteUser, createUserManually } from "./users-actions";
import { Button } from "@/components/ui/button";
import { Input, Label, Select } from "@/components/ui/input";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { cn } from "@/lib/utils";

export function InviteUserForm() {
  const [mode, setMode] = useState<"convite" | "manual">("manual");

  return (
    <Card>
      <CardHeader>
        <CardTitle>Adicionar utilizador</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="mb-4 flex gap-1 rounded-md bg-background p-1">
          <button
            type="button"
            onClick={() => setMode("manual")}
            className={cn(
              "flex-1 rounded-[6px] px-2.5 py-1.5 text-[12.5px] font-medium transition-colors",
              mode === "manual"
                ? "bg-surface text-foreground shadow-sm"
                : "text-foreground-faint hover:text-foreground"
            )}
          >
            Criar diretamente
          </button>
          <button
            type="button"
            onClick={() => setMode("convite")}
            className={cn(
              "flex-1 rounded-[6px] px-2.5 py-1.5 text-[12.5px] font-medium transition-colors",
              mode === "convite"
                ? "bg-surface text-foreground shadow-sm"
                : "text-foreground-faint hover:text-foreground"
            )}
          >
            Convidar por email
          </button>
        </div>

        {mode === "manual" ? <CreateUserForm /> : <SendInviteForm />}
      </CardContent>
    </Card>
  );
}

function CreateUserForm() {
  const [state, formAction, pending] = useActionState<
    { error: string | null; ok: boolean },
    FormData
  >(createUserManually, { error: null, ok: false });

  return (
    <form action={formAction} className="space-y-3" key={state.ok ? "reset" : "form"}>
      <div>
        <Label htmlFor="c_full_name">Nome</Label>
        <Input id="c_full_name" name="full_name" required />
      </div>
      <div>
        <Label htmlFor="c_email">Email</Label>
        <Input id="c_email" name="email" type="email" required />
      </div>
      <div>
        <Label htmlFor="c_password">Palavra-passe</Label>
        <Input id="c_password" name="password" type="password" minLength={6} required />
      </div>
      <div>
        <Label htmlFor="c_role">Função</Label>
        <Select id="c_role" name="role" defaultValue="professional">
          <option value="professional">Profissional</option>
          <option value="admin">Administrador</option>
        </Select>
      </div>
      {state.error && (
        <p className="rounded-md bg-rose-soft px-3 py-2 text-[12.5px] text-rose">{state.error}</p>
      )}
      {state.ok && (
        <p className="rounded-md bg-accent-soft px-3 py-2 text-[12.5px] text-accent-ink">
          Utilizador criado. Já pode entrar com o email e a palavra-passe definidos.
        </p>
      )}
      <Button type="submit" className="w-full" size="sm" disabled={pending}>
        {pending ? "A criar…" : "Criar utilizador"}
      </Button>
    </form>
  );
}

function SendInviteForm() {
  const [state, formAction, pending] = useActionState<
    { error: string | null; ok: boolean },
    FormData
  >(inviteUser, { error: null, ok: false });

  return (
    <form action={formAction} className="space-y-3">
      <div>
        <Label htmlFor="full_name">Nome</Label>
        <Input id="full_name" name="full_name" required />
      </div>
      <div>
        <Label htmlFor="email">Email</Label>
        <Input id="email" name="email" type="email" required />
      </div>
      <div>
        <Label htmlFor="role">Papel</Label>
        <Select id="role" name="role" defaultValue="professional">
          <option value="professional">Profissional</option>
          <option value="admin">Administrador</option>
        </Select>
      </div>
      {state.error && (
        <p className="rounded-md bg-rose-soft px-3 py-2 text-[12.5px] text-rose">{state.error}</p>
      )}
      {state.ok && (
        <p className="rounded-md bg-accent-soft px-3 py-2 text-[12.5px] text-accent-ink">
          Convite enviado por email.
        </p>
      )}
      <Button type="submit" className="w-full" size="sm" disabled={pending}>
        {pending ? "A enviar…" : "Enviar convite"}
      </Button>
    </form>
  );
}
